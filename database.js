class Database {
    ...
    async openDB() {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                // إنشاء مخزن للمسعفين
                if (!db.objectStoreNames.contains('medics')) {
                    const medicsStore = db.createObjectStore('medics', { keyPath: 'id' });
                    medicsStore.createIndex('code', 'code', { unique: false });
                    medicsStore.createIndex('discord_id', 'discord_id', { unique: false });
                    medicsStore.createIndex('role', 'role', { unique: false });
                    medicsStore.createIndex('name', 'name', { unique: false });
                }

                // إنشاء مخزن لجلسات المسعفين
                if (!db.objectStoreNames.contains('medic_sessions')) {
                    const sessionsStore = db.createObjectStore('medic_sessions', { keyPath: 'id', autoIncrement: true });
                    sessionsStore.createIndex('medic_id', 'medic_id');
                    sessionsStore.createIndex('status', 'status');
                }

                // إنشاء مخزن لساعات المسعفين
                if (!db.objectStoreNames.contains('medic_hours')) {
                    const hoursStore = db.createObjectStore('medic_hours', { keyPath: 'id', autoIncrement: true });
                    hoursStore.createIndex('medic_id', 'medic_id');
                    hoursStore.createIndex('date', 'date');
                }

                // إنشاء مخزن للنقاط
                if (!db.objectStoreNames.contains('points')) {
                    const pointsStore = db.createObjectStore('points', { keyPath: 'id', autoIncrement: true });
                    pointsStore.createIndex('medic_id', 'medic_id');
                }
            };
            ...
            request.onsuccess = async (event) => {
                ...
                // إنشاء حساب وزير الصحة الافتراضي إذا لم يكن موجودًا قبل المتابعة
                await this.ensureDefaultMinister();
    ...
    // التأكد من وجود حساب وزير الصحة الافتراضي
    async ensureDefaultMinister() {
        try {
            const store = await this.getStore('medics', 'readwrite');
            const index = store.index('role');
            const existing = await new Promise((resolve, reject) => {
                const req = index.get('minister');
            ...
            if (!existing) {
                ...
                store.put(defaultMinister);
                console.log('تم إضافة حساب وزير الصحة الافتراضي');
            }
        } catch (error) {
            console.error('خطأ في إنشاء حساب وزير الصحة الافتراضي:', error);
        }
    }
    ...
    async addMedic(medicData) {
        try {
            const store = await this.getStore('medics', 'readwrite');
            return new Promise((resolve, reject) => {
                const request = store.put(medicData); // put يسمح بالإضافة أو التحديث إذا كان المفتاح موجودًا
                request.onsuccess = () => resolve(medicData);
                request.onerror = () => reject(request.error);
            });
        } catch (error) {
            console.error('خطأ في إضافة/تحديث المسعف:', error);
            throw error;
        }
    }
    ...
    async deleteMedic(medicId) {
        try {
            const store = await this.getStore('medics', 'readwrite');
            return new Promise((resolve, reject) => {
                const request = store.delete(medicId);
                request.onsuccess = () => resolve();
                request.onerror = () => reject(request.error);
            });
        } catch (error) {
    ...
    async getMedicById(medicId) {
        try {
            const store = await this.getStore('medics', 'readonly');
            return new Promise((resolve, reject) => {
                const request = store.get(medicId);
                request.onsuccess = () => resolve(request.result);
                request.onerror = () => reject(request.error);
            });
        } catch (error) {
            console.error('خطأ في جلب المسعف:', error);
            throw error;
        }
    }
    ...
    async getAllMedics() {
        try {
            ...
            console.error('خطأ في جلب جميع المسعفين:', error);
            throw error;
        }
    }

    async getMedicByCode(code) {
        try {
            const store = await this.getStore('medics', 'readonly');
            ...
            console.error('خطأ في جلب المسعف بواسطة الكود:', error);
            throw error;
        }
    }
    ...
    async addPoint(pointData) {
        try {
            const store = await this.getStore('points', 'readwrite');
            return new Promise((resolve, reject) => {
                const request = store.add(pointData);
                request.onsuccess = () => resolve(request.result);
                request.onerror = () => reject(request.error);
            });
        } catch (error) {
    ...
    async getPointsByMedic(medicId) {
        try {
            const store = await this.getStore('points', 'readonly');
            return new Promise((resolve, reject) => {
                const index = store.index('medic_id');
                const request = index.getAll(IDBKeyRange.only(medicId));
                request.onsuccess = () => resolve(request.result);
                request.onerror = () => reject(request.error);
            });
        } catch (error) {
    ...
    // ==================== CRUD: جلسات المسعفين ====================
    async upsertMedicSession(sessionData) {
        try {
            const store = await this.getStore('medic_sessions', 'readwrite');
            return new Promise((resolve, reject) => {
    ...
    async getActiveSessionByMedic(medicId) {
        try {
            const store = await this.getStore('medic_sessions', 'readonly');
            const index = store.index('medic_id');
            return new Promise((resolve, reject) => {
                const request = index.getAll(IDBKeyRange.only(medicId));
                request.onsuccess = () => {
    ...
    // ==================== CRUD: ساعات المسعفين ====================
    async upsertMedicHours(hoursData) {
        try {
            const store = await this.getStore('medic_hours', 'readwrite');
            return new Promise((resolve, reject) => {
                const request = store.put(hoursData);
                request.onsuccess = () => resolve(hoursData);
                request.onerror = () => reject(request.error);
            });
        } catch (error) {
            console.error('خطأ في حفظ ساعات المسعف:', error);
            throw error;
        }
    }

    async getHoursByMedicAndDate(medicId, date) {
        try {
            const store = await this.getStore('medic_hours', 'readonly');
            // استخدام اندكس مزدوج لو وجد لكن هنا نبحث يدويًا
            return new Promise((resolve, reject) => {
                const index = store.index('medic_id');
                const request = index.getAll(IDBKeyRange.only(medicId));
                request.onsuccess = () => {
                    const records = request.result || [];
                    const matched = records.find(r => r.date === date);
                    resolve(matched || null);
                };
                request.onerror = () => reject(request.error);
            });
        } catch (error) {
            console.error('خطأ في جلب ساعات المسعف:', error);
            throw error;
        }
    }
